...
const inputCls = 'w-full px-3 py-2 rounded-lg bg-[#f5f5f7] border border-black/10 text-[#1d1d1f] placeholder-[#86868b] focus:outline-none focus:border-[#e8590c]';
...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#fbfbfd] text-[#1d1d1f] flex items-start justify-center px-4 py-10">
      <div className="w-full max-w-2xl">

        {/* Kopf + Fortschritt */}
        <div className="text-center mb-8">
          <div className="text-blue-600 text-sm font-semibold tracking-widest mb-2">SCAFFOLD OS</div>
          <h1 className="text-3xl font-bold">Willkommen! Richten wir Ihr System ein.</h1>
          <p className="text-[#86868b] mt-2">Drei kurze Schritte – danach ist alles einsatzbereit.</p>
          <div className="flex justify-center gap-2 mt-6">
            {[1, 2, 3].map((s) => (
              <div key={s} className={`h-2 w-16 rounded-full ${s <= schritt ? 'bg-blue-500' : 'bg-black/10'}`} />
            ))}
          </div>
          <div className="text-[#86868b] text-sm mt-2">Schritt {schritt} von 3: {schrittTitel[schritt]}</div>
        </div>

        {fehler && (
          <div className="mb-4 px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-sm">{fehler}</div>
        )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {FIRMEN_FELDER.map((f) => (
                <div key={f.key} className={f.key === 'depot_address' || f.key === 'company_name' ? 'sm:col-span-2' : ''}>
                  <label className="block text-sm text-[#424245] mb-1">
                    {f.label} {f.pflicht && <span className="text-blue-600">*</span>}
                  </label>
                  <input className={inputCls} value={firm[f.key]} onChange={(e) => setF(f.key, e.target.value)} />
                  {f.hint && <p className="text-xs text-[#86868b] mt-1">{f.hint}</p>}
                </div>
              ))}
            </div>
            <button onClick={speichernFirma} disabled={saving}
              className="mt-6 w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 font-semibold disabled:opacity-50">
        ...
        {schritt === 2 && (
          <div className="bg-black/5 border border-black/10 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-[#424245] mb-1">Fahrzeug (z. B. Mercedes Sprinter)</label>
                  <input className={inputCls} value={fahrzeug} onChange={(e) => setFahrzeug(e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm text-[#424245] mb-1">Kennzeichen (für GPS)</label>
                  <input className={inputCls} value={kennzeichen} onChange={(e) => setKennzeichen(e.target.value)} />
                </div>
              </div>
              <div>
                <label className="block text-sm text-[#424245] mb-1">Fahrer (Name)</label>
                <input className={inputCls} value={fahrer} onChange={(e) => setFahrer(e.target.value)} />
              </div>
            </div>
            <div className="mt-6 flex gap-3">
              <button onClick={() => setSchritt(1)}
                className="flex items-center gap-2 px-4 py-3 rounded-xl bg-black/10 hover:bg-black/15">
                <ArrowLeft size={18} /> Zurück
              </button>
              <button onClick={speichernStammdaten} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 font-semibold disabled:opacity-50">
        ...
        {schritt === 3 && (
          <div className="bg-black/5 border border-black/10 rounded-2xl p-6">
            ...
            {demoGeladen ? (
              <div className="mb-5 px-4 py-3 rounded-xl bg-green-900/40 border border-green-700 text-green-200 text-sm flex items-center gap-2">
                <CheckCircle size={18} /> {demoInfo}
              </div>
            ) : (
              <button onClick={ladeDemo} disabled={saving}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-black/10 hover:bg-black/15 border border-black/10 disabled:opacity-50">
            ...
            <div className="mt-6 flex gap-3">
              ...
              <button onClick={abschliessen} disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-green-600 hover:bg-green-500 font-semibold disabled:opacity-50">
