...
export default function KundenDetailPage() {
  ...
  const ladeDaten = useCallback(async () => {
    setLoading(true)
    try {
      const [kRes, pRes, iRes, eRes] = await Promise.all([
        fetch(`/api/kunden?id=${id}`),
        fetch(`/api/projects?customer_id=${id}`),
        fetch(`/api/invoices`),
        fetch(`/api/email-log?customer_id=${id}`),
      ])
      ...
      if (kJson.success) setKunde(kJson.kunden?.[0] || null)
      if (pJson.success) setProjects(pJson.projects || [])
      if (iJson.success) setInvoices(iJson.invoices || [])
