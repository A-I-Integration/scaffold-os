...
export default function DispositionResult({ result, loading }: Props) {
  if (loading) {
    return (
      <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
        ...
        <p className="text-lg font-semibold text-white">Optimiere Transporte...</p>
        <p className="text-sm text-slate-400">Prüfe Lager, Baustellen und Routen</p>
      </div>
    );
  }
  ...
  const sourceColors: Record<string, string> = {
    ...
    site_via_central: 'bg-amber-500/20 text-amber-300 border-amber-500/30',
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-6">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">KI-Disposition</h2>
            <p className="text-slate-400">Optimale Materialbeschaffung & Transport</p>
          </div>
        </div>
        <p className="text-emerald-200">{result.summaryText}</p>
      </div>
      ...
      {result.totalSavings > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="rounded-lg bg-slate-800 border border-emerald-500/30 p-4 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Einsparung</p>
          </div>
          <div className="rounded-lg bg-slate-800 border border-emerald-500/30 p-4 text-center">
            <p className="text-2xl font-bold text-emerald-400">{result.totalSavedKm} km</p>
            <p className="text-xs text-slate-400 mt-1">gesparte Strecke</p>
          </div>
          <div className="rounded-lg bg-slate-800 border border-emerald-500/30 p-4 text-center">
            <p className="text-2xl font-bold text-emerald-400">{result.totalSavedDiesel} L</p>
            <p className="text-xs text-slate-400 mt-1">Diesel gespart</p>
          </div>
          <div className="rounded-lg bg-slate-800 border border-emerald-500/30 p-4 text-center">
            <p className="text-2xl font-bold text-emerald-400">{result.totalSavedHours} h</p>
            <p className="text-xs text-slate-400 mt-1">Zeit gespart</p>
          </div>
        </div>
      )}
      ...
      {result.routes.length > 0 && (
        <div className="rounded-xl border border-slate-700 bg-slate-800 p-6">
          <h3 className="text-lg font-bold text-white mb-4">🗺️ Optimierte Routen</h3>
      ...
      <div className="rounded-xl border border-slate-700 bg-slate-800">
        <div className="border-b border-slate-700 p-4">
          <h3 className="text-lg font-bold text-white">Positionen im Detail</h3>
        </div>
            ...
            <div key={i} className="p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-semibold text-white">{s.articleName}</span>
                  <span className="text-slate-500 text-sm">({s.articleNumber})</span>
                </div>
                <span className="text-white font-bold">{s.needed} {s.needed === 1 ? 'Stk' : 'Stk'}</span>
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <button className="flex-1 rounded-lg bg-emerald-600 hover:bg-emerald-500 py-3 font-semibold text-white transition-colors">
          🚛 Transportaufträge erstellen
        </button>
        <button className="flex-1 rounded-lg bg-slate-700 hover:bg-slate-600 py-3 font-semibold text-white transition-colors">
