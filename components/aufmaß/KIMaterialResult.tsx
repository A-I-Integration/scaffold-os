    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-black/10 bg-[#f5f5f7] p-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-xl bg-black/10/50 p-3">
            <p className="text-xs text-[#86868b]">Gerüstklasse</p>
            <p className="text-lg font-bold text-[#1d1d1f]">
              {result.scaffoldClass}
            </p>
          </div>
          <div className="rounded-xl bg-black/10/50 p-3">
            <p className="text-xs text-[#86868b]">Gesamtgewicht</p>
            <p className="text-lg font-bold text-[#1d1d1f]">
              {result.totalWeightKg.toLocaleString('de-DE')} kg
            </p>
          </div>
          <div className="rounded-xl bg-black/10/50 p-3">
            <p className="text-xs text-[#86868b]">Arbeitsstunden</p>
            <p className="text-lg font-bold text-[#1d1d1f]">
              {result.estimatedLaborHours} h
            </p>
          </div>
          <div className="rounded-xl bg-black/10/50 p-3">
            <p className="text-xs text-[#86868b]">Anker erforderlich</p>
            <p className="text-lg font-bold text-[#1d1d1f]">
      ...
      {result.warnings.length > 0 && (
        <div className="space-y-2">
          {result.warnings.map((w, i) => (
            <div
              key={i}
              className="flex items-start gap-3 rounded-xl border border-[#e8590c]/30 bg-[#e8590c]/10 p-4"
            >
      ...
      <div className="rounded-xl border border-black/10 bg-[#f5f5f7]">
        <div className="border-b border-black/10 p-4">
          <h3 className="text-lg font-bold text-[#1d1d1f]">Materialliste</h3>
          <p className="text-sm text-[#86868b]">
            ...
            return (
              <div key={category}>
                <button
                  ...
                  className="flex w-full items-center justify-between p-4 text-left transition-colors hover:bg-black/10/30"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-[#1d1d1f]">{category}</span>
                    <span className="rounded-full bg-black/10 px-2 py-0.5 text-xs text-[#424245]">
                      {items.length}
                    </span>
                  </div>
                ...
                {isOpen && (
                  <div className="bg-white/50 px-4 pb-4">
                        ...
                        <tbody className="divide-y divide-black/5">
                          {items.map((item, idx) => (
                            <tr key={idx} className="group">
                              <td className="py-2 font-mono text-[#86868b]">
                                {item.articleNumber}
                              </td>
                              <td className="py-2">
                              ...
                              <td className="py-2 text-right font-bold text-[#e8590c]">
      ...
      {/* --- KOSTENÜBERSICHT --- */}
      <div className="rounded-xl border border-black/10 bg-[#f5f5f7] p-6">
        <h3 className="mb-4 text-lg font-bold text-[#1d1d1f]">Kostenkalkulation</h3>
        ...
        <div className="space-y-3">
          ...
          <div className="border-t border-black/10 pt-3">
            <div className="flex justify-between text-lg font-bold text-[#1d1d1f]">
              ...
              <span className="font-mono text-[#e8590c]">
          ...
          <div className="rounded-xl bg-emerald-500/10 p-4">
            ...
            <div className="mt-1 flex justify-between text-sm text-emerald-700/70">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <button 
          ...
          className="flex-1 rounded-xl bg-blue-600 py-3 font-semibold text-white transition-colors hover:bg-blue-500 disabled:opacity-50"
        ...
        <button 
          ...
          className="flex-1 rounded-xl bg-orange-600 py-3 font-semibold text-white transition-colors hover:bg-orange-500 disabled:opacity-50"
        ...
        <button 
          ...
          className="flex-1 rounded-xl bg-black/10 py-3 font-semibold text-[#1d1d1f] transition-colors hover:bg-black/15 disabled:opacity-50"
